...
function proknjizi_kompenzaciju($id_kompenzacije, $datum_naloga) {
    ...
    //DOBIJANJE PRVOG SLOBODNOG DATUMA NAKON ZAKLJUCANOG PERIODA
    $min_datum = proveriZDatum('KVART', '1970-01-01');

    //AKO JE DATUM NALOGA MANJI OD PRVOG DATUMA POSLE ZAKLJUCANOG PERIODA, PROMENI VREDNOST NA TAJ DATUM
    if ($datum_naloga < $min_datum) {

        $datum_naloga = $min_datum;
    }
    ...
    if ($flag) {
        ...
        if (!$rezultat_podaci) {
            ...
            $upit_knjizenje = "INSERT INTO g$godina (datknjiz, vrstadok, brdok, ff, partner, pib, ppsi, klgod, klbroj, klvaluta, kldatum, opisdok, brojdok, datdok,
             
            dospeva, dugdeviz, potrazdeviz, duguje, potrazuje, opetnalog, konto, mnt, radnik, knjizdana, vremknjiz) VALUES ";
            ...
            foreach($niz_podaci as $stavka_kompenzacije) {
                ...
                //AKO JE DOKUMENT STETA,DODAJ NAZIV PARTNERA U OPISDOK
                if ($brojdok[0] == 'A') {

                    $opis_dokumenta = $stavka_kompenzacije['naziv_partnera'] .' '. $stavka_kompenzacije['opisdok'];
                }
                //U SUPROTNOM UPISI SAMO OPISDOK
                else {
                    $opis_dokumenta = $stavka_kompenzacije['opisdok'];
                }
                ...
                if ($stavka_kompenzacije['konto'][0] == '4' && $stavka_kompenzacije['duguje'] > 0) {
                ...
                $upit_knjizenje .= "('$datum_naloga', 'KM', '$brdok', 'F', '$pib_partnera', $kanal_prodaje, 'PP', NULL, NULL, NULL, NULL, '$opis_dokumenta',
                '$brojdok', '$datum_naloga', '$datum_naloga', NULL, NULL, '$duguje', '$potrazuje', '$brdok', '$konto', '111000', $radnik, current_date, current_time),";
